### Задание 1. - Создание базы данных для хранения результатов анкетирования
...
5. 
6.
7. Добавьте еще одну таблицу (2-ю)
```
Название:
product
Поля (Columns):
...
sum FLOAT
```
Сохраните модель `File \ Save model` как `IS221`
<hr>
...
(указав название скрипта `IS221`, два раза нажмите `Next` и один раз `Finish`)  
(если ругается на права - скопируйте через буфер обмена и создайте файл через Блокнот)  
...
(назовите его `IS221-connection` и нажмите `OK`)
...
### Задание 5. - Вставка данных в таблицу order (invoice)

1. Откройте вкладку для записи запроса `File \ New Query Tab` и вставьте 3 записи языком запросов SQL в таблицу `order (invoice)`
```
INSERT INTO .. (.., ..) VALUES (1, 1, 300);
INSERT INTO .. (.., ..) VALUES (1, 2, 350);
INSERT INTO .. (.., ..) VALUES (1, 3, 400);

вместо многоточия (..) добавьте название таблицы и названия списка полей (без id)
```
